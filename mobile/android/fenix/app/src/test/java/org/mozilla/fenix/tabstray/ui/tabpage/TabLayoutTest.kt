...
class TabLayoutTest {
    ...
    private fun rowsNeededFor(itemCount: Int, columns: Int): Int =
        ceil(itemCount.toDouble() / columns).toInt()

    private val gridCollectionInfo: CollectionInfo
        get() = composeTestRule.onNodeWithTag(TabsTrayTestTag.TAB_GRID)
            .fetchSemanticsNode().config[SemanticsProperties.CollectionInfo]

    private val listCollectionInfo: CollectionInfo
        get() = composeTestRule.onNodeWithTag(TabsTrayTestTag.TAB_LIST)
            .fetchSemanticsNode().config[SemanticsProperties.CollectionInfo]
