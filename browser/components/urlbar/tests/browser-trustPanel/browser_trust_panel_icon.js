...
add_task(async function test_tab_switch_back_shows_correct_count() {
  ...
  try {
    ...
    // Read the live count right before switching away. The count can include
    // both the explicitly triggered cryptominer and any other origins that
    // were logged asynchronously (e.g. the page's static trackertest.org
    // iframe).
    Assert.greater(
      Number(
        document.getElementById("trust-icon-tracker-count-shortform")
