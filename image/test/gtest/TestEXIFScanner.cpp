...
///////////////////////////////////////////////////////////////////////////////
// Sniffing. Formats we do not walk still have to be identified.
///////////////////////////////////////////////////////////////////////////////
...
// Every brand the AV1 Image File Format defines, against the HEIF brands that
// share the container. Only the major brand is read, so this is also what pins
// that: a file is judged on the four bytes at offset 8 and nothing else.
TEST(EXIFScanner, ISOBaseMediaBrandsSplitAVIFFromHEIF)
{
  auto withBrand = [](const char* aBrand) {
    nsTArray<uint8_t> data;
    AppendLiteral(data,
                  "\x00\x00\x00\x1C"
                  "ftyp",
                  8);
    AppendLiteral(data, aBrand, 4);
    return data;
  };

  for (const char* brand : {"avif", "avis", "avio", "miaf"}) {
    nsTArray<uint8_t> data = withBrand(brand);
    EXPECT_EQ(EXIFContainer::AVIF, Scan(data).container) << brand;
  }

  for (const char* brand : {"heic", "heix", "hevc", "mif1", "msf1"}) {
    nsTArray<uint8_t> data = withBrand(brand);
    EXPECT_EQ(EXIFContainer::HEIF, Scan(data).container) << brand;
  }
}

// "GIF8" alone is not a signature: the version and the trailing "a" are part of
// it, so a file that only shares the first four bytes must not be labelled GIF.
TEST(EXIFScanner, GIFNeedsItsWholeSignature)
{
  for (const char* signature : {"GIF87a", "GIF89a"}) {
    nsTArray<uint8_t> data;
    AppendLiteral(data, signature, 6);
    EXPECT_EQ(EXIFContainer::GIF, Scan(data).container) << signature;
  }

  nsTArray<uint8_t> impostor;
  AppendLiteral(impostor, "GIF8\x00\x00", 6);
  EXPECT_EQ(EXIFContainer::Unknown, Scan(impostor).container);
}
