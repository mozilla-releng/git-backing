...
void RecordEvent(ImageInputSource aInputType, const nsACString& aDetectedFormat,
                 const nsACString& aHasGPS, const nsACString& aScanOutcome) {
...
void ImageInputScan::RecordSkipped(ImageInputSource aInputType,
                                   const nsACString& aScanOutcome) {
  RecordEvent(aInputType, "not_scanned"_ns, "unknown"_ns, aScanOutcome);
}
...
void ImageInputScan::Start(dom::BlobImpl* aBlob, ImageInputSource aInputType) {
  ...
  // so this is a backstop against a caller added later that does not.
  ...
  if (rv.Failed()) {
    RecordSkipped(aInputType, "read_error"_ns);
    return;
  }
  if (size == 0) {
    RecordSkipped(aInputType, "skipped_empty_file"_ns);
    return;
  }
  ...
  RefPtr<ImageInputScan> scan = new ImageInputScan(aInputType, size);
...
void ImageInputScan::OnBytes(nsresult, nsTArray<uint8_t>&& aBytes) {
  ...
  if (aBytes.IsEmpty()) {
    RecordSkipped(mInputType, "read_error"_ns);
    return;
  }
  ...
  RecordEvent(mInputType, imageinput::DetectedFormatLabel(result.container),
