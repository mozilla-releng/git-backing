...
add_task(async function recording_resumes_when_enabled() {
  // Set explicitly rather than leaning on the default, so that flipping the
  // default off cannot turn this into a test that asserts nothing.
  await SpecialPowers.pushPrefEnv({
    set: [["privacy.imageInputTelemetry.enabled", true]],
  });
