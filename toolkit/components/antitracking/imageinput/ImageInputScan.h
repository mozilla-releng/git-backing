...
class ImageInputScan final {
 public:
  ...
  // Changing either cap means retuning image_input.batch_size: its buckets are
  // chosen so that one past each cap, 9 and 33, is a bucket boundary, which is
  // what makes "this selection lost detail" exactly countable.
 ...
 private:
  ImageInputScan(ImageInputSource aInputType, uint64_t aFileSize)
      : mInputType(aInputType), mFileSize(aFileSize) {}
