...
async function navigateLinkClick(
  ...
  let clicked = await SpecialPowers.spawn(
    ...
    async (targetURL, spawnWindow, linkTarget) => {
      ...
      // getBoundingClientRect flushes layout, so the box is already final.
      // Check it rather than clicking blindly, which would dispatch at (0, 0)
      // and hit the document instead of the link.
      let { width, height } = link.getBoundingClientRect();
      if (!width || !height) {
        throw new Error(`The link was not laid out: ${width}x${height}`);
      }
