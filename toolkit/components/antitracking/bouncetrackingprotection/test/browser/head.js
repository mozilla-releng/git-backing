...
async function navigateLinkClick(
  ...
  let clicked = await SpecialPowers.spawn(
    ...
    async (targetURL, spawnWindow, linkTarget) => {
      ...
      // getBoundingClientRect flushes layout, so this resolves on the first
      // check unless the link never gets a frame. Waiting here rather than
      // clicking blindly turns that case into a clear failure instead of a
      // click dispatched at (0, 0).
      await ContentTaskUtils.waitForCondition(() => {
        let { width, height } = link.getBoundingClientRect();
        return width > 0 && height > 0;
      }, "The link has been laid out.");
