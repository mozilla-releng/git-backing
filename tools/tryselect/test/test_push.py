...
def test_push_to_git_backing_key_usable(tmp_path, monkeypatch, mock_tc_secret):
    ...
    def check_keyfile(*args, **kwargs):
        ...
        if sys.platform == "win32":
            ...
            # server. Inspect the ACL directly instead: none of the broad
            # groups a temp file inherits by default should have access.
            acl = subprocess.run(
                ["icacls", keyfile_path],
                capture_output=True,
                text=True,
            ...
            too_open_markers = ("everyone", "authenticated users", "\\users:")
            if any(marker in acl for marker in too_open_markers):
