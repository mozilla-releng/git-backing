...
def fake_auth(monkeypatch):
    def make(
        disabled=False, expires_offset_s=TC_CREDENTIALS_EXPIRY_DAYS * 86400, exc=None
    ):
        class _FakeAuth:
            ...
            def client(self, client_id):
                if exc is not None:
                    raise exc
                return {"disabled": disabled, "expires": _expires_iso(expires_offset_s)}
...
@pytest.fixture
def run_get_client(monkeypatch, fake_auth):
    fake_auth()
...
def test_get_client_cache_expired(credentials_file, run_get_client, fake_auth):
    make_cache(credentials_file)
    fake_auth(expires_offset_s=200)
    result = run_get_client()
    assert isinstance(result, tc_module.Queue)
    assert result.options["rootUrl"] == TC_ROOT_URL
...
def test_get_client_deleted_tc_client(credentials_file, run_get_client, fake_auth):
    make_cache(credentials_file)
    fake_auth(
        exc=TaskclusterRestFailure(
            "Client not found", superExc=None, status_code=404, body={}
        )
    )
    result = run_get_client()
    assert result.options["credentials"]["clientId"] == b"browser-client"
...
def test_get_client_auth_failure(credentials_file, run_get_client, fake_auth):
    make_cache(credentials_file)
    fake_auth(
        exc=TaskclusterAuthFailure("Forbidden", status_code=401, body={}, superExc=None)
    )
    result = run_get_client()
    assert result.options["credentials"]["clientId"] == b"browser-client"


def test_get_client_disabled(credentials_file, run_get_client, fake_auth):
    make_cache(credentials_file)
    fake_auth(disabled=True)
    result = run_get_client()
    assert result.options["credentials"]["clientId"] == b"browser-client"
