...
struct ComponentNameEx {
  CacheableName name;
  ComponentNameAttributes attributes;

  explicit ComponentNameEx() = default;
  explicit ComponentNameEx(CacheableName&& name,
                           ComponentNameAttributes attributes)
...
class ComponentImport {
  ComponentNameEx name_;
  ComponentExternDesc externDesc_;

 public:
  explicit ComponentImport(ComponentNameEx&& name,
                           const ComponentExternDesc& externDesc)
      : name_(std::move(name)), externDesc_(externDesc) {}

  const ComponentNameEx& name() const { return name_; }
...
class ComponentExport {
 ...
 public:
  explicit ComponentExport(ComponentNameEx&& name,
